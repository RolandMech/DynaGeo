...

{
  ...
  Eine Schwierigkeit bei der diesen Kodierungen ist das Problem einer 
  unvollständigen 3-Byte-Gruppe (am Ende der zu verschlüsselnden Daten) 
  bzw. 4-Char-Gruppe (am Ende der verschlüsselten Daten). Der hier 
  verwendete Algorithmus verzichtet auf eine spezielle Behandlung dieser
  "überzähligen Reste", weil dies überflüssig ist: wieviele Zeichen der 
  codierte String lang sein muss, ergibt sich eindeutig aus der Anzahl 
  der zu verschlüsselnden Bytes. Und umgekehrt gehört zu jeder 
  möglichen(!) Länge des codierten Strings eindeutig die Anzahl der in 
  diesem String verschlüsselten Bytes. Genauer: ist n die Anzahl der zu 
  kodierenden Bytes und t die Länge des kodierten Strings, dann gilt 
  (mit möglichst großem k) :
